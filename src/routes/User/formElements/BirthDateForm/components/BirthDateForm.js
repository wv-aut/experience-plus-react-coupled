...
export const BirthDateForm = (props, { store }) => {
  ...
  const _checkIfBoolean = value => {
    if (typeof value === 'boolean') {
      console.log(value.toString() === 'false')
  ...
  const checkIfBirthDateIsRequired = (classValue, ifNot = false) => {
    if (!props.user.data.taxOptOut) {
      return classValue
    } else if (ifNot) {
      return classValue
    }
  ...
  return (
    <div>
      ...
      <div className={'form-row ' + checkIfBirthDateIsRequired('required')}>
        ...
        <label className='grid-3-all'>
          <span className={checkIfBirthDateIsRequired('optional', true)}>Monat:</span>
          <select
